...
import accentMap from './accentMap.js';
...
export default class ReactUIDropdown extends Component {
  ...
  componentWillReceiveProps(nextProps){
    for (let i = 0; i < nextProps.prints.length; i++){
      this.addItemToSelected(nextProps.prints[i])
    }
  }
  ...
  goSearch(q, callback) {
    ...
    this.state.items.keys.all.forEach(itemKey => {
      if (itemChecker.check(this.accent_fold(q), this.state.items.collection[itemKey], fields)) foundItemsKeys.push(itemKey);
    });
  ...
  render() {
    ...
    return (
      <div className="dropdown">
        <Label labelStyle={this.props.labelStyle} idPrefix={dropdownId}>{this.props.label}</Label>
        ...
        <div
          ...
          <SearchInput
            ...
            buyPrints={this.props.buyPrints}
          />
        </div>
        <Items
          ref="items"
